...
const ScriptLoader = () => {
  ...
  const initializeScripts = async () => {
    try {
      ...
      if (typeof window !== 'undefined' && (window as any).jQuery) {
        ...
        // Product slider initialization is now handled by specific components
        // NewArrivals slider is handled by useNewArrivalsSlider hook
        ...
        // Modal image sliders are now handled by QuickViewModal component
        // No need to initialize here as it's handled in the modal component

        // Initialize quantity buttons for static content with delay to prevent duplicates
        setTimeout(() => {
        ...
        $('#showlogin').off('click').on('click', function(e: any) {
          e.preventDefault();
          $('#checkout-login').slideToggle();
        });

        $('#showcoupon').off('click').on('click', function(e: any) {
          e.preventDefault();
          $('#checkout_coupon').slideToggle();
        });

        // Initialize payment accordion
        $('.payment-accordion .panel-title a').off('click').on('click', function(e: any) {
          e.preventDefault();
        ...
        $('#ship-box').off('change').on('change', function(this: any) {
          if ($(this).is(':checked')) {
        ...
        // Initialize color swatches for static content
        $('.single-color').off('click').on('click', function(this: any, e: any) {
          e.preventDefault();
        ...
        $('.price-range').off('input').on('input', function(this: any) {
          const value = $(this).val();
          $('.min-price').val(value);
        });

        $('.color-filter input, .size-filter input').off('change').on('change', function(this: any) {
          // Handle filter changes
          console.log('Filter changed:', $(this).val());
        });

        // Initialize contact form
        $('.contact-form form').off('submit').on('submit', function(e: any) {
          e.preventDefault();
        ...
        $('#form-review').off('submit').on('submit', function(this: any, e: any) {
          e.preventDefault();
  ...
  useEffect(() => {
    if (!scriptsLoaded.current) {
      ...
      // If scripts are already loaded, just reinitialize
      initializeScripts();
    }
  }, [pathname]);
