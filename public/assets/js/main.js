...
(function ($) {
  ...
  // This is the core function that will contain all of your jQuery initialization logic.
  // It is exposed globally so that it can be called by a Next.js component.
  window.mainJsInit = function () {
    /*-----------------------------
		Uren's Window When Loading
---------------------------------*/
    // The "load" event is unreliable in Next.js's client-side routing.
    // The preloader logic has been moved here to ensure it runs on every page.
    $(".loading").fadeOut(500);
...
/*----------------------------------------*/
    // The scroll listener needs to be set up every time the page loads.
    $(window)
      .off("scroll")
      .on("scroll", function () {
    ...
    /* Uren's Main Slider
/*----------------------------------------*/
    // Initialize the main slider only if it exists and is not already initialized
    if (
      $(".main-slider").length &&
      !$(".main-slider").hasClass("slick-initialized")
    ) {
      try {
        $(".main-slider").slick({
          infinite: true,
        ...
        $(".main-slider").addClass("slick-initialized");
        console.log("Main slider initialized from main.js");
      } catch (error) {
        console.error("Error initializing main slider from main.js:", error);
      }
    }
    ...
    /* Toolbar Button
/*----------------------------------------*/
    // Unbind and rebind to prevent multiple event listeners
    $(".toolbar-btn")
      .off("click")
      .on("click", function (e) {
        e.preventDefault();
...
/*----------------------------------------*/
    ...
    $offcanvasNav
      .off("click")
      .on("click", "li a, li .menu-expand", function (e) {
        var $this = $(this);
        if (
          $this
            .parent()
...
/*----------------------------------------*/
    ...
    $(".category-menu-list")
      .off("click")
      .on("click", "li a, li a .menu-expand", function (e) {
...
/*----------------------------------------*/
    ...
    if ($elementCarousel.elExists()) {
      ...
      $elementCarousel.each(function (index, element) {
        ...
        // Skip if already initialized by React or if no slick options
        if (
          $this.hasClass("slick-initialized") ||
          $this.hasClass("blog-slider")
        ) {
          return;
        }
        ...
        try {
          ...
          console.log("Carousel initialized:", $this.attr("class"));
        } catch (error) {
          console.error(
...
/*----------------------------------------*/
    // Initialize single blog slider only if it exists and is not already initialized
    if (
      $(".single-blog_slider").length &&
      !$(".single-blog_slider").hasClass("slick-initialized")
    ) {
      try {
        ...
        $(".single-blog_slider").addClass("slick-initialized");
        console.log("Single blog slider initialized from main.js");
      } catch (error) {
        console.error(
          "Error initializing single blog slider from main.js:",
          error
        );
      }
    }
 ...
 /*----------------------------------------*/
  ...
  // This IIFE will run automatically on the first page load.
  // It checks if jQuery is available, and then calls the main initialization function.
  $(function () {
    if (typeof window.mainJsInit === "function") {
      window.mainJsInit();
    }
  });
})(jQuery);
